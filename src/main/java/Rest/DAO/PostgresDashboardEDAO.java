...
public class PostgresDashboardEDAO implements DashboardEDAO {

	private JdbcTemplate jdbcTemplate;

	public PostgresDashboardEDAO(JdbcTemplate jdbcTemplate) {
	...
	public int registeredPets() {
		String query = "SELECT * FROM animal WHERE castration = true ;";
	...
	public long countryOriginCount() {
		String query = "SELECT COUNT(DISTINCT origin_country_code) as count FROM animal;";
	...
	public long doneExaminatons() {
		String query = "SELECT COUNT(id_exam) as count FROM examination as e "
				...
				+ "WHERE e.when_date < CURRENT_DATE;";
		long result = jdbcTemplate.queryForObject(query, new RowMapper<Long>() {
			@Override
			public Long mapRow(ResultSet rs, int rowNum) throws SQLException{
				long id = rs.getLong("count");
